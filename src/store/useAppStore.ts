  ...
  persist(
    (set, get) => ({
      ...
      playGacha: () => {
        ...
        // 該当レアリティのアイテムから選択
        const availableItems = state.gachaItems.filter(
          ...
          Object.keys(newMissions.weekly).forEach((missionId) => {
            ...
            if (!mission.completed && mission.type === actionType) {
              if (actionType === "total_savings") {
                ...
                const weekCooking = state.cookingRecords.filter(
                  (record) =>
                    record.date >= weekStart &&
                    record.date < weekEnd.toISOString().split("T")[0]
                ).length;
      ...
      claimMissionReward: (missionId) => {
        const state = get();
        const mission =
          state.missions.daily[missionId] || state.missions.weekly[missionId];
      ...
      checkBadgeProgress: () => {
          ...
          if (!state.badges.earned.includes(badge.id)) {
            ...
            switch (req.type) {
              ...
              case "consecutive_days":
                // 連続記録日数計算（簡略化）
                shouldEarn =
                  state.expenses.length > 0 || state.cookingRecords.length > 0;
                break;
              ...
              case "missions_completed":
                shouldEarn =
                  state.missions.completedHistory.length >= req.value;
                break;
            }
        ...
        if (newBadges.length > 0) {
          set((state) => {
            ...
            const earnedBadgeObjects = badgeDefinitions.filter((b) =>
              earnedBadges.includes(b.id)
            );
            earnedBadgeObjects.sort((a, b) => {
              const priorityDiff =
                priorities[b.category] - priorities[a.category];
      ...
      recordNoWasteDay: () => {
        ...
        set((state) => {
          const newStreak =
            state.streaks.lastNoWasteDate === yesterdayStr
          ...
          return {
            streaks: {
              ...
              bestNoWasteStreak: Math.max(
                newStreak,
                state.streaks.bestNoWasteStreak
              ),
            },
            userData: {
              ...state.userData,
      ...
      recordSnackFreeDay: () => {
        ...
        set((state) => {
          const newStreak =
            state.streaks.lastSnackFreeDate === yesterdayStr
          ...
          return {
            streaks: {
              ...
              bestSnackFreeStreak: Math.max(
                newStreak,
                state.streaks.bestSnackFreeStreak
              ),
            },
            userData: {
              ...state.userData,
      ...
      updateMonthlyData: () => {
        ...
        const monthlyCooking = state.cookingRecords.filter((r) =>
          r.date.startsWith(currentMonth)
        );
        const monthlySavings = state.savingsRecords.filter((r) =>
          r.date.startsWith(currentMonth)
        );
        ...
        const allowanceUsed = wastefulExpenses.reduce(
          (sum, e) => sum + e.amount,
          0
        );
        const cookingCount = monthlyCooking.length;
        const monthlySavingsAmount = monthlySavings.reduce(
          (sum, r) => sum + r.amount,
          0
        );
