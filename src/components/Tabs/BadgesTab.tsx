...
export const BadgesTab: React.FC = () => {
  ...
  const getConsecutiveDays = (): number => {
    if (expenses.length === 0 && cookingRecords.length === 0 && savingsRecords.length === 0) {
      return 0;
    }
  ...
  const checkMonthlyGoalAchieved = (): boolean => {
    // 簡略化した目標達成判定
    return userData.monthlyExpense <= 25000 && 
  ...
  const getCurrentTitle = (): string => {
    const currentBadge = badgeDefinitions.find(b => b.id === badges.currentTitle);
  ...
  const completionRate = Math.round((earnedBadges.length / badgeDefinitions.length) * 100);
    ...
    <section className="tab-content active">
      <div className="badges-header">
        ...
        <div className="badges-stats">
          <span>獲得称号: {earnedBadges.length} / {badgeDefinitions.length}</span>
          <span>達成率: {completionRate}%</span>
        </div>
        <div className="current-title">
          現在の称号: {getCurrentTitle()}
        </div>
      </div>
      ...
      <div className="badge-categories">
        ...
        <button 
          className={`category-filter-btn ${selectedCategory === 'cooking' ? 'active' : ''}`}
          onClick={() => handleCategoryFilter('cooking')}
        >
          🍳 自炊
        </button>
        <button 
          className={`category-filter-btn ${selectedCategory === 'savings' ? 'active' : ''}`}
          onClick={() => handleCategoryFilter('savings')}
        >
          💰 節約
        </button>
        <button 
          className={`category-filter-btn ${selectedCategory === 'level' ? 'active' : ''}`}
          onClick={() => handleCategoryFilter('level')}
        >
          ⭐ レベル
        </button>
        <button 
          className={`category-filter-btn ${selectedCategory === 'special' ? 'active' : ''}`}
      ...
      <div className="badges-grid">
        {filteredBadges.map(badge => {
          ...
          const progressPercent = isEarned ? 100 : Math.min((progress / badge.requirement.value) * 100, 100);
