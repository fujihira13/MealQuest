...
class FoodExpenseApp {
  constructor() {
    ...
    this.badgeDefinitions = [
      // 自炊関連
      {
        ...
        description: "初回自炊記録",
        icon: "🍳",
        requirement: { type: "cooking_count", value: 1 },
        earned: false,
      },
      {
        id: "cooking_novice",
        ...
        description: "自炊5回達成",
        icon: "👨‍🍳",
        requirement: { type: "cooking_count", value: 5 },
        earned: false,
      },
      {
        id: "cooking_adept",
        ...
        description: "自炊20回達成",
        icon: "🧑‍🍳",
        requirement: { type: "cooking_count", value: 20 },
        earned: false,
      },
      {
        id: "cooking_master",
        ...
        description: "自炊50回達成",
        icon: "👑",
        requirement: { type: "cooking_count", value: 50 },
        earned: false,
      },
      {
        id: "cooking_legend",
        category: "cooking",
        title: "料理の達人",
        description: "自炊100回達成",
        icon: "🌟",
        requirement: { type: "cooking_count", value: 100 },
        earned: false,
      },
      ...
      },
      {
        ...
        category: "savings",
        title: "節約マスター",
        description: "10000円節約達成",
        icon: "👑",
        requirement: { type: "total_savings", value: 10000 },
        earned: false,
      },
      {
        id: "savings_champion",
        category: "savings",
        title: "節約チャンピオン",
        description: "30000円節約達成",
        icon: "🏆",
        requirement: { type: "total_savings", value: 30000 },
        earned: false,
      },
      {
        id: "savings_legend",
        category: "savings",
        title: "節約の王様",
        description: "50000円節約達成",
        icon: "👑",
        requirement: { type: "total_savings", value: 50000 },
        earned: false,
      },
      ...
      // 節約レベル関連
      {
        ...
        category: "savings",
        title: "節約ビギナー",
        description: "節約レベル5達成",
        icon: "⭐",
        requirement: { type: "savings_level", value: 5 },
        earned: false,
      },
      {
        id: "savings_level_10",
        category: "savings",
        title: "節約アドバンス",
        description: "節約レベル10達成",
        icon: "⭐⭐",
        requirement: { type: "savings_level", value: 10 },
        earned: false,
      },
      {
        id: "savings_level_20",
        category: "savings",
        title: "節約プロ",
        description: "節約レベル20達成",
        icon: "⭐⭐⭐",
        requirement: { type: "savings_level", value: 20 },
        earned: false,
      },
      ...
      // 連続記録関連
      {
        ...
        category: "special",
        title: "継続の力",
        description: "7日連続無駄遣いなし",
        icon: "🔥",
        requirement: { type: "no_waste_streak", value: 7 },
        earned: false,
      },
      {
        id: "streak_month",
        category: "special",
        title: "鉄の意志",
        description: "30日連続無駄遣いなし",
        icon: "💪",
        requirement: { type: "no_waste_streak", value: 30 },
        earned: false,
      },
      ...
      },
      {
        ...
        description: "月間目標達成",
        icon: "🎯",
        requirement: { type: "monthly_goal_achieved", value: 1 },
        earned: false,
      },
      {
        id: "gacha_collector",
        category: "special",
        title: "コレクター",
        description: "ガチャアイテム10種獲得",
        icon: "🎁",
        requirement: { type: "gacha_items", value: 10 },
        earned: false,
      },
      {
        id: "mission_master",
        category: "special",
        title: "クエストマスター",
        description: "クエスト20個達成",
        icon: "🏅",
        requirement: { type: "missions_completed", value: 20 },
        earned: false,
      },
    ];
  ...
  updateAmountDisplay() {
    const display = this.currentAmount
      ? parseInt(this.currentAmount).toLocaleString()
      : "";
  ...
  saveExpenseRecord() {
    ...
    this.showNotification(
      `${
        this.currentInputCategory
      }の支出を記録しました: ¥${amount.toLocaleString()}`
    );
  }
  ...
  recordCooking(meal, button) {
    ...
    const existingRecord = this.cookingRecords.find(
      (r) => r.date === today && r.meal === meal
    );

    if (existingRecord) {
      // 既に記録済みの場合は削除
      this.cookingRecords = this.cookingRecords.filter(
        (r) => !(r.date === today && r.meal === meal)
      );
      button.classList.remove("active");
      this.showNotification(
        `${this.getMealName(meal)}の自炊記録を削除しました`
      );
    } else {
      // 新規記録
      const record = {
      ...
      this.showNotification(
        `${this.getMealName(meal)}の自炊を記録しました！ +20pt`
      );
  ...
  recordSavings(amount) {
    ...
    this.saveData();
    this.showNotification(
      `節約成功！ ¥${amount.toLocaleString()}を節約貯金に追加しました！ +${Math.floor(
        amount / 10
      )}pt`
    );
  }
  ...
  playGacha() {
    ...
    // 該当レアリティのアイテムから選択
    const availableItems = this.gachaItems.filter(
    ...
    // コレクションに追加
    const existingItem = this.collection.find(
    ...
    this.showNotification(
      `ガチャ結果: ${rarityText[selectedRarity]} ${selectedItem.name} ${
        selectedItem.icon
      }を獲得！${
        bonusPoints[selectedRarity] > 0
          ? ` (+${bonusPoints[selectedRarity]}pt)`
          : ""
      }`
    );
  }
  ...
  updateExpenseData() {
    ...
    this.userData.monthlyExpense = monthlyExpenses.reduce(
      (sum, e) => sum + e.amount,
      0
    );

    // 消費許容額の更新（スーパー以外）
    const wastefulExpenses = monthlyExpenses.filter(
      (e) => e.category !== "スーパー"
    );
    this.userData.allowanceUsed = wastefulExpenses.reduce(
      (sum, e) => sum + e.amount,
      0
    );
  }
  ...
  updateCookingData() {
    const currentMonth = new Date().toISOString().slice(0, 7);
    const monthlyCooking = this.cookingRecords.filter((r) =>
  ...
  updateSavingsData() {
    const currentMonth = new Date().toISOString().slice(0, 7);
    const monthlySavingsRecords = this.savingsRecords.filter((r) =>
      r.date.startsWith(currentMonth)
    );
    this.userData.monthlySavings = monthlySavingsRecords.reduce(
      (sum, r) => sum + r.amount,
      0
    );
  }
  ...
  updateUI() {
    ...
    const remaining = Math.max(
      0,
      this.goals.allowanceGoal - this.userData.allowanceUsed
    );
    const gaugePercent = Math.max(
      0,
      (remaining / this.goals.allowanceGoal) * 100
    );
    document.getElementById("allowance-gauge").style.width = `${gaugePercent}%`;
    document.getElementById("remaining-allowance").textContent =
      remaining.toLocaleString();
    document.getElementById("total-allowance").textContent =
      this.goals.allowanceGoal.toLocaleString();

    // アバター・ステータス
    document.getElementById("savings-total").textContent =
      this.userData.totalSavings.toLocaleString();
    ...
    if (monthlySavingsElement) {
      monthlySavingsElement.textContent =
        this.userData.monthlySavings.toLocaleString();
    }
    ...
    document.getElementById("monthly-expense").textContent =
      this.userData.monthlyExpense.toLocaleString();
    ...
    document.getElementById("total-expense").textContent =
      this.userData.monthlyExpense.toLocaleString();
    document.getElementById("expense-goal").textContent =
      this.goals.monthlyExpenseGoal.toLocaleString();
    document.getElementById("total-savings").textContent =
      this.userData.totalSavings.toLocaleString();
    document.getElementById("cooking-total").textContent =
      this.userData.cookingCount;
    document.getElementById("cooking-goal").textContent =
      this.goals.cookingGoal;
  ...
  checkSavingsLevelUp() {
    ...
    if (this.userData.totalSavings >= requiredSavings) {
      ...
      if (newLevel > this.userData.savingsLevel) {
        ...
        this.showNotification(
          `節約レベルアップ！ 節約Lv.${this.userData.savingsLevel}になりました！ (+${bonus}pt)`,
          "success"
        );
        this.updateBadgeProgress();
      }
  ...
  loadRecordsForDate() {
    ...
    const dayCooking = this.cookingRecords.filter(
    ...
    if (dayExpenses.length === 0 && dayCooking.length === 0) {
      recordList.innerHTML =
        '<p class="no-records">この日の記録はありません</p>';
      return;
    }
    ...
    if (type === "expense") {
      item.innerHTML = `
                <div class="record-info">
                    <span class="record-category">${this.getCategoryIcon(
                      record.category
                    )} ${record.category}</span>
                    <span class="record-amount">¥${record.amount.toLocaleString()}</span>
                    <span class="record-time">${this.getMealName(
                      record.meal
                    )}</span>
                </div>
                <div class="record-actions">
                    <button class="edit-btn" onclick="app.editRecord(${
                      record.id
                    }, 'expense')">編集</button>
                    <button class="delete-btn" onclick="app.deleteRecord(${
                      record.id
                    }, 'expense')">削除</button>
                </div>
            `;
      ...
      item.innerHTML = `
                <div class="record-info">
                    ...
                    <span class="record-amount">${this.getMealName(
                      record.meal
                    )}</span>
    ...
    if (type === "expense") {
      ...
      if (record) {
        ...
        setTimeout(() => {
          ...
          const mealBtn = document.querySelector(
  ...
  updateCollectionDisplay() {
    ...
    // 統計更新
    if (collectionCount) {
      collectionCount.textContent = this.collection.reduce(
    ...
    // フィルター取得
    const activeFilter =
      document.querySelector(".filter-btn.active")?.dataset.rarity || "all";
    ...
    const itemsToShow =
      activeFilter === "all"
        ? this.gachaItems
        : this.gachaItems.filter((item) => item.rarity === activeFilter);

    if (itemsToShow.length === 0) {
      collectionGrid.innerHTML =
        '<div class="collection-empty"><div class="collection-empty-icon">📦</div><p>アイテムがありません</p></div>';
      return;
    }
    ...
    itemsToShow.forEach((gachaItem) => {
      ...
      itemElement.className = `collection-item rarity-${gachaItem.rarity} ${
        !isObtained ? "locked" : ""
      }`;
      ...
      itemElement.innerHTML = `
                ...
                <div class="item-rarity">${this.getRarityText(
                  gachaItem.rarity
                )}</div>
  ...
  renderCookingChart() {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      // その日の自炊回数を計算
      const dayCount = this.cookingRecords.filter(
    ...
    return this.savingsRecords
      .filter(
        (record) =>
          ...
          record.date < weekEnd.toISOString().split("T")[0]
      )
  ...
  getWeekCookingCount() {
    ...
    return this.cookingRecords.filter(
      (record) =>
        record.date >= weekStart &&
        record.date < weekEnd.toISOString().split("T")[0]
    ).length;
  ...
  createMissionElement(mission) {
    ...
    element.className = `mission-card ${mission.completed ? "completed" : ""} ${
            ...
            <div class="mission-action">
                ${
                  mission.completed && !mission.claimed
                    ...
                    : mission.claimed
                    ? '<span class="claimed-text">受取済</span>'
                    : ""
                }
            </div>
        `;
  ...
  claimMissionReward(missionId) {
    const mission =
      this.missions.daily[missionId] || this.missions.weekly[missionId];
  ...
  updateMissionTimers() {
    ...
    if (dailyTimer) {
      dailyTimer.textContent = `${hours.toString().padStart(2, "0")}:${minutes
        .toString()
  ...
  getConsecutiveDays() {
    if (
      ...
      this.cookingRecords.length === 0 &&
      this.savingsRecords.length === 0
    ) {
      return 0;
    }
  ...
  updateCurrentTitle() {
    ...
    earnedBadges.sort((a, b) => {
      ...
      return (
        this.badges.earned.indexOf(b.id) - this.badges.earned.indexOf(a.id)
      );
    });
  ...
  updateCurrentTitleDisplay() {
    ...
    const currentBadge = this.badgeDefinitions.find(
      (b) => b.id === this.badges.currentTitle
    );
  ...
  filterBadgesByCategory(category, button) {
    ...
    badges.forEach((badge) => {
      if (
        category === "all" ||
        badge.classList.contains(`category-${category}`)
      ) {
  ...
  recordNoWasteDay() {
    ...
    if (this.streaks.noWasteStreak > this.streaks.bestNoWasteStreak) {
      ...
      this.showNotification(
        `🎉 無駄遣いなし記録更新！${this.streaks.noWasteStreak}日連続達成！`,
        "success"
      );
    } else {
      this.showNotification(
        `🔥 無駄遣いなし ${this.streaks.noWasteStreak}日連続！素晴らしい！`,
        "success"
      );
    }
  ...
  recordSnackFreeDay() {
    ...
    if (this.streaks.snackFreeStreak > this.streaks.bestSnackFreeStreak) {
      ...
      this.showNotification(
        `🎉 お菓子我慢記録更新！${this.streaks.snackFreeStreak}日連続達成！`,
        "success"
      );
    } else {
      this.showNotification(
        `🍭 お菓子我慢 ${this.streaks.snackFreeStreak}日連続！頑張ってる！`,
        "success"
      );
    }
  ...
  updateSavingsEquivalent() {
    ...
    if (bestMatch) {
      ...
      element.innerHTML = `あと¥${remaining.toLocaleString()}で${
        cheapest.icon
      }<strong>${cheapest.item}</strong>が買えます！`;
    }
  }
  ...
  updateSavingsGoals() {
    ...
    const goals = [
      {
        ...
        progress: "short-term-progress",
        target: 5000,
      },
      {
        current: "medium-term-current",
        progress: "medium-term-progress",
        target: 20000,
      },
      {
        current: "long-term-current",
        progress: "long-term-progress",
        target: 50000,
      },
    ];
    ...
    goals.forEach((goal) => {
      ...
      if (currentElement) {
        currentElement.textContent = Math.min(
          totalSavings,
          goal.target
        ).toLocaleString();
      }
