---
name: codex-review
description: Codex CLI（read-only）を用いて、レビュー→Codex修正→再レビュー（ok: true まで）を反復し収束させるレビューゲート。プラン確定後（ExitPlanMode後）、各実装ステップ完了後（規模問わず）、テスト実行後、および git commit / PR / merge / release 前に使用する。キーワード: Codexレビュー, codex review, レビューゲート.
---
...
規模判定 → Codex規模別レビュー → Codex修正 → 再レビュー（`ok: true`まで反復）
...
1. `issues`解析 → 修正計画
2. Codexが修正（最小差分のみ、仕様変更は未解決issueに）
3. テスト/リンタ実行（可能なら）
4. Codexに再レビュー依頼
...
CodexにJSON1つのみ出力させる。Codexはプロンプト末尾に以下のスキーマとフィールド説明を添付。
...
- `notes_for_next_review`: Codexが残すメモ。再レビュー時にCodexがプロンプトに含める
